...
def create_features(tweets,tfidf,off_words_cv,url_cv,
                    do_text_clean=False,need_tfidf=True):
...
def fit_tfidf(tweets,name):
    tfidf=TfidfVectorizer(stop_words='english')
    vect=tfidf.fit_transform(tweets)
    with open(r'F:\E\Learning_DL_fastai\competition\NLP_data\model_files\{}.pkl'.format(name),'wb') as f:
        pickle.dump(tfidf,f)
    return tfidf,vect.toarray()
...
if __name__=='__main__':
    ...
    sdf=create_features(tweet_df['tweet'],tfidf,off_words_cv,url_cv)
    sdf.to_csv('fe_check.csv')
